...
const CommunityWorkspaceSettings = memo(() => {
    ...
    <Flexbox gap={12}>
      ...
      <Flexbox gap={4}>
        <Text strong as="h2" style={{ fontSize: 20, margin: 0 }}>
          {t('user.workspaceProfile.settings.title')}
        </Text>
  ...
  if (!profile) {
    return (
      <PageContainer>
        {renderHeader(t('user.workspaceProfile.settings.noProfile.description'))}
      </PageContainer>
    );
  }
      ...
      >
        <Input
          ...
          onChange={(e) => {
            ...
            setWebsiteUrl(next);
            setWebsiteError(isValidUrl(next) ? undefined : t('user.workspaceProfile.errors.url'));
          }}
        />
      </SettingCard>
