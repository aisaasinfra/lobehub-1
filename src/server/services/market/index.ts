...
export class MarketService {
  ...
  async uploadCredFile(params: {
    ...
    log('uploadCredFile: fileName=%s, fileType=%s, orgId=%s', fileName, fileType, orgId ?? 'none');
    ...
    for (const [key, value] of Object.entries(sdkHeaders)) {
      ...
      if (lower === 'x-lobe-owner-account-id' && orgId !== undefined) continue;
    ...
    const uploadPath =
      orgId === undefined
        ...
        : `/api/v1/organizations/${orgId}/creds/upload`;
